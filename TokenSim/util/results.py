...
def get_batch_stats(engine: LLMEngine, duration: float) -> dict[str, Any]:
    """Time-weighted concurrent batch size and HBM occupancy.

    ``max_parallem_sum`` caps concurrent requests PER SCHEDULER
    (``llm_scheduler.py:198-201``), and each data-parallel group has its own
    scheduler, so both a per-scheduler and a cluster-wide figure are reported.
    """
    workers = [
        worker
        for worker in engine.workers
        if getattr(worker.scheduler, "block_manager", None) is not None
    ]
    if not workers or duration <= 0:
        return {}
    for worker in workers:
        worker.flush_batch_state(duration)

    blocks_per_rank = workers[0].scheduler.block_manager.num_total_gpu_blocks
    batch_area = sum(worker._batch_area for worker in workers)
    occupancy_area = sum(worker._occupancy_area for worker in workers)
    peak_used = max(worker.max_used_blocks_observed for worker in workers)

    return {
        "batch_scheduler_count": len(workers),
        "batch_num_gpu_blocks_per_rank": int(blocks_per_rank),
        "batch_mean_per_scheduler": batch_area / duration / len(workers),
        "batch_mean_cluster": batch_area / duration,
        "batch_peak_per_scheduler": max(
            worker.max_batch_observed for worker in workers
        ),
        "batch_peak_cluster": sum(worker.max_batch_observed for worker in workers),
        "hbm_mean_used_blocks_per_rank": occupancy_area / duration / len(workers),
        "hbm_mean_occupancy_frac": (
            occupancy_area / duration / len(workers) / blocks_per_rank
        ),
        "hbm_peak_used_blocks_per_rank": int(peak_used),
        "hbm_peak_occupancy_frac": peak_used / blocks_per_rank,
    }
...
def export_result(
    ...
    batch_stats = get_batch_stats(engine, duration)
