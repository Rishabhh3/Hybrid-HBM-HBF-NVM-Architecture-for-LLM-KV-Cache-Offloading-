...
class MetricData:
    ...
    p99: float
    max: float
    # Mean is reported alongside the percentiles: TBT and TTFT are quoted as
    # means in the HBF-endurance literature, and a p50-only record cannot be
    ...
    @classmethod
    def inf(cls):
        return cls(999999, 999999, 999999, 999999)
...
class LLMResult:
    ...
    mooncake_memory_eviction_count: int = 0
    mooncake_ssd_eviction_count: int = 0
    ...
    mooncake_wasted_write_blocks: int = 0
    mooncake_useful_write_blocks: int = 0
    mooncake_mean_residency_s: float = 0
    mooncake_residency_histogram: dict[str, int] | None = None
    mooncake_blocks_dropped: int = 0
    mooncake_blocks_demoted: int = 0
    mooncake_memory_bypass_blocks: int = 0
    mooncake_memory_write_blocks: int = 0
    mooncake_memory_write_bytes: int = 0
    mooncake_memory_read_blocks: int = 0
    mooncake_memory_read_bytes: int = 0
    mooncake_memory_write_latency: float = 0
    mooncake_memory_read_latency: float = 0
    batch_scheduler_count: int = 0
    batch_num_gpu_blocks_per_rank: int = 0
    batch_mean_per_scheduler: float = 0
    batch_mean_cluster: float = 0
    batch_peak_per_scheduler: int = 0
    batch_peak_cluster: int = 0
    hbm_mean_used_blocks_per_rank: float = 0
    hbm_mean_occupancy_frac: float = 0
    hbm_peak_used_blocks_per_rank: int = 0
    hbm_peak_occupancy_frac: float = 0
