...
class LLMWorker(Worker):
    def __init__(
        ...
        # Batch-size / HBM-occupancy instrumentation. Sampled once per STEP
        # and integrated over simulated time, so the means below are
        # time-weighted, not per-step averages (steps are not equal length).
        self._batch_area: float = 0.0
        self._occupancy_area: float = 0.0
        self._batch_last_t: float = 0.0
        self._batch_current: int = 0
        self._occupancy_current: int = 0
        self.max_batch_observed: int = 0
        self.max_used_blocks_observed: int = 0

        self.status = WorkerStatus.WAITING
        self.action = self.env.process(self.run())

    def sample_batch_state(self) -> None:
        """Accumulate the time-weighted concurrent-request count and HBM use.

        Called immediately after each scheduling decision. Workers with no
        block manager (non-paged-attn schedulers) are skipped.
        """
        block_manager = getattr(self.scheduler, "block_manager", None)
        if block_manager is None:
            return
        now = self.env.now
        delta = now - self._batch_last_t
        if delta > 0:
            self._batch_area += self._batch_current * delta
            self._occupancy_area += self._occupancy_current * delta
        self._batch_last_t = now
        self._batch_current = len(self.scheduler.running)
        _, used_blocks, _ = block_manager.get_gpu_status()
        self._occupancy_current = used_blocks
        self.max_batch_observed = max(self.max_batch_observed, self._batch_current)
        self.max_used_blocks_observed = max(
            self.max_used_blocks_observed,
            used_blocks,
        )

    def flush_batch_state(self, duration: float) -> None:
        """Close the final sampling interval at end of simulation."""
        delta = duration - self._batch_last_t
        if delta > 0:
            self._batch_area += self._batch_current * delta
            self._occupancy_area += self._occupancy_current * delta
        self._batch_last_t = duration
